...
[s3; Содержание&]
[s0; &]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`$ru`-ru`#1^ 1. Обработчики 
SKYLARK и паттерны пути]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#2^ 2. Witz templates]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#3^ 3. Witz links 
to handlers]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#4^ 4. Combining 
Witz templates using #define and #include]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#5^ 5. FORM GET]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#6^ 6. FORM POST]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#7^ 7. Session 
variables]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#8^ 8. Ajax support]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#9^ 9. Connecting 
SQL database]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#10^ 10. Advanced 
SQL]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#11^ 11. Language 
support]&]
[s0; [^topic`:`/`/Skylark`/srcdoc`/Tutorial`_ru`-ru`#12^ 12. Packs]&]
[s0; &]
[s3;:1: 1. 1. Обработчики SKYLARK и паттерны 
пути&]
[s5; Приложение Skylark подразделено на `'обработчик
и`', которые обеспечивают ответ на 
разные запросы по HTTP, основанные на 
паттерне url (УЛР `- универсальный локатор 
ресурса):&]
[s7; &]
...
[s7; [* SKYLARK](HomePage, `"`")&]
[s7; `{&]
[s7; -|http << `"<!DOCTYPE html><html><body>Привет мир!</body></html>`";&]
[s7; `}&]
[s7; &]
[s7; [* SKYLARK](Param, `"`*/param`")&]
[s7; `{&]
[s7; -|http << `"<!DOCTYPE html><html><body>Парметр: `" << http`[0`] 
<< `"</html></body>`";&]
[s7; `}&]
[s7; &]
...
[s7; `}&]
[s7; &]
[s5; Обработчики вводятся макросом [@5 SKYLARK], 
первый параметр представляет имя 
обработчика (макрос создаёт функцию, 
названную именно так, а её идентификатор 
также используется в различных местах, 
типа шаблонов страниц), и паттерном 
пути, по которому предполагается 
ответ обработчика.&]
[s5; Обработчик представлен функцией 
C`+`+ с одним параметром Http`& [/ http], который 
используется для обработки запроса. 
Макрос [@5 SKYLARK] создаёт для этой функции 
заголовок, а также регистрирует его 
в Skylark, после чего запросы, соответствующие 
паттерну пути, будут отправляться 
этой функции. Важное примечание: так 
как макрос [@5 SKYLARK] использует глобальные 
конструкторы при регистрации обработчиков, 
обработчики следует помещать в файлы 
.icpp (если они не расположены в одном 
файле с функцией `'main`').&]
[s5; Паттерн пути может содержать местодержател
и параметра `'`*`' (смотрите [@5 Param] handler), 
which are then provided by [/ http] as parameters (through operator`[`](int)). 
Path is always split by `'/`' characters and each part must either 
be text or placeholder `- patterns like `"something`*/to`*avoid`" 
are not allowed&]
...
[s7; &]
[s5; &]
[s3; Http request type handler:&]
[s5; Previously we saw how to deal with [* GET ]and [* POST ]request 
with special handler.&]
[s5; It is possible in skylark to deal with many more kind of HTTP 
request using special handler. Here are few examples:&]
[s5;%- [* OPTIONS handler:]&]
[s7; &]
[s0;l321; [C@5+75 SKYLARK(OptionsHandler, `"my`_options:OPTIONS`")`{]&]
[s7; -|...&]
[s7; `}&]
[s5;%- [* DELETE handler:]&]
[s7; &]
[s0;l321; [C@5+75 SKYLARK(DeleteHandler, `"my`_delete:DELETE`")`{]&]
[s0;l321; [C@5+75 -|...]&]
[s7; `}&]
[s5;%- [* CONNECT handler:]&]
[s7; &]
[s0;l321; [C@5+75 SKYLARK(ConnectHandler, `"my`_connect:CONNECT`")`{]&]
[s0;l321; [C@5+75 -|...]&]
[s7; `}&]
[s5; Currently all the following list of Http request can be handled 
this way:&]
[s5; [C@5+75 -|HttpMethod`{ GET, POST, OPTIONS, PUT, HEAD, CONNECT, 
TRACE, PATCH, POST`_RAW`};]&]
[s5;C@5+75 &]
[s3; Recommended tutorials:&]
[s5; If you want to learn more, we have several tutorials that you 
can find useful:&]
[s5;l160;i150;O0; [^topic`:`/`/Sql`/srcdoc`/tutorial`_ru`-ru^ SQL tutorial] 
